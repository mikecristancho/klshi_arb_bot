...
# ==================== CONFIGURATION ====================
TEST_MODE = True  # Set True to test with a single dummy order; False for full arb mode
...
# For test mode: A real market ticker (change if needed; this is for a political event)
TEST_TICKER = "TRUMPWIN-28"  # Example - use a current market from Kalshi app
# ======================================================

# Debug startup - print ASAP
...
def sign_request(method, path):
    ...
    print(f"Signature generated for {method} {path}")
...
def kalshi_request(method, endpoint, json_body=None):
    ...
    try:
        ...
        print(f"{method} {endpoint} response: {resp.status_code}")
        resp.raise_for_status()
        return resp.json()
    except Exception as e:
        print(f"REQUEST FAILED: {e} | Text: {getattr(resp, 'text', 'No text')}")
        raise
...
def execute_test_trade():
    print("TEST MODE ACTIVE - Placing dummy order on " + TEST_TICKER)
    test_payload = {"ticker": TEST_TICKER, "action": "buy", "type": "limit", "count": 1, "side": "yes", "yes_price": 1}  # Bad price - won't fill
    kalshi_request("POST", "/orders", test_payload)
    print("Test order placed - check Kalshi account for pending order (cancel it if needed)")

print("STARTUP COMPLETE - BOT READY")
